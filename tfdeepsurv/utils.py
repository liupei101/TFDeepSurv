...
def readData(file, name, discount, tgt, seed=1):
    random.seed(seed)
    # Read data
    ...
    a=data.pop(tgt[0])
    data.insert(len(list(data)), tgt[0], a)
    b=data.pop(tgt[1])
    data.insert(len(list(data)), tgt[1], b)
    ...
    sampleM[:, :1] = (sampleM[:, :1] - sampleM[:, :1].min()) / (sampleM[:, :1].max() - sampleM[:, :1].min())
